...
import { useGetPortfolioCategoriesQuery } from "#store/api/portfolioCategory";
...
const PortfolioIsotop = () => {
  ...
  // category list
  const { data: portfolioCategory, isLoading, isError, error } = useGetPortfolioCategoriesQuery(null);
  const { data: portfolioItem } = useGetPortfoliosQuery(null);

  console.log({ portfolioCategory, isLoading, isError, error });
      ...
      {isLoading && !isError ? (
        ...
        <section id="portfolio" className="portfolio section-gap base-bg-light">
          <div className="container">
            ...
            {/* <!-- Filter Menu --> */}
            <ul id="portfolio-flters" className="border-bottom-0 mb-4">
              <li className={`nav-link ${selectedCategory === "All" ? "filter-active" : ""}`} onClick={() => setSelectedCategory("All")}>
                All
              </li>
              {portfolioCategory.data.map((category: any, i: number) => {
                return (
                  <li
                    key={i}
            ...
            <div className="portfolio popup-ajax-gallery">
              <div className="row portfolio-container">
                {portfolioItem?.data
                  ...
                  .map((item: any, i: any) => {
                    return (
                      <div className="col-lg-4 col-md-6 portfolio-item mb-4 filter-app" key={i}>
                        <div className="portfolio-wrap" data-aos="zoom-in" data-aos-duration="1000">
